...
export function ScoreBadge({ score, className = "", confidence }: Props) {
  ...
  return (
    <span
      ...
      title={lowConf ? "Score basé sur des données partielles (confiance surface faible)" : undefined}
    >
      <span className="tabular-nums">{rounded}</span>
      <span className="font-normal opacity-80">· {label}</span>
      {lowConf && (
        <span className="ml-0.5 inline-flex h-3 w-3 items-center justify-center rounded-full bg-amber-500/80 text-[8px] font-bold text-white" aria-label="Confiance faible">
          !
        </span>
      )}
    </span>
