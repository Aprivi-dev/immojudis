...
_Mis à jour le 30 juillet 2026. Ce fichier distingue les capacités réellement présentes dans le dépôt des phases cibles du cahier des charges. Une table créée ne signifie pas que son workflow est opérationnel; une UI visible ne signifie pas que la donnée est disponible à l’échelle nationale._
...
Le worktree prépare une **tranche verticale partielle** : audit et contrats, sous-ensemble de fondation relationnelle du registre, baseline descriptive déterministe, contrôle d’accès premium et restitution en lecture sur la fiche Analyse. Il contient aussi une première chaîne d’ingestion Outcome : connecteurs et parseurs DVF/Judilibre/Justice/Enchères, provenance append-only, bucket privé, file de jobs, checkpoints et CLI bornée. Les 84 migrations locales ont été rejouées depuis zéro et les suites SQL ont réussi, mais les migrations Outcome Graph et leur backfill ne sont pas déployés sur le Supabase distant. Une composante n’est considérée livrée ci-dessous qu’après présence de son implémentation et vérification correspondante; la checklist factuelle prévaut sur ce résumé.
...
### Fondation et registre : préparatoire, SQL présent
...
Le schéma reste qualifié de **préparatoire pour la production**, malgré un replay local depuis zéro des 84 migrations et un résultat pgTAP global de 274/274 : les migrations ne sont pas appliquées sur le Supabase distant et aucun backfill distant n’a été effectué. L’ensemble reste un sous-ensemble du dictionnaire cible : organisations et rôles fins, parcelles/bâtiments normalisés, observations de participation, qualité/audit et plusieurs invariants de workflow restent à ajouter. Le contrat SQL de prédiction sépare le report de l’annulation/non-réquisition, mais valide encore P10/P50/P90 seulement, alors que la cible porte la grille P05…P95.

Ne sont pas inclus par ces migrations : le backfill national en base distante, les formulaires opérateur, le workflow complet de revue, le scheduler régulier, le worker de purge physique et la résolution automatique des conflits. Aucun artefact n’a été envoyé au Supabase distant dans cet environnement. Le déploiement devra coordonner le code du pipeline et les migrations : la garde de suppression du pont est volontairement fail-closed.
...
- Pont catalogue : quel que soit leur statut — notamment `upcoming`, `past` ou `adjudicated` — les lignes sont préservées comme annonces avec résultat `unknown`, sans prix et avec `training_eligible = false`. Le catalogue distant compte actuellement 413 lignes, dont 4 affichent un prix d’adjudication, mais ces quatre valeurs ne constituent pas des preuves et ne sont pas promues dans Outcome Graph. Le backfill distant n’a pas été exécuté.
...
| 1 — Fondation        | **Préparatoire**          | Migration additive de 16 tables, RLS sans grant navigateur, validations d’insertion et append-only; replay local et pgTAP réussis        | Déploiement coordonné en staging puis production, monorepo cible, pnpm/uv, rôles complets, stockage preuve et observabilité dédiée                               |
| 2 — Registre         | **Préparatoire**          | Dossier, lot, round, événements, versions de résultat, preuves/revues et pont catalogue fail-closed testés localement                    | CRUD complet, entités auxiliaires, backfill distant vérifié et parcours E2E sans SQL direct                                                                      |
...
| 5 — Sources ouvertes | **Partiel**               | Connecteurs/parseurs DVF, Judilibre, Justice et Enchères; données réelles contrôlées; provenance, bucket privé et politiques fail-closed | Déployer code et migrations ensemble, fournir les credentials PISTE, approuver le premier sync live ciblé, puis organiser imports distants et collecte régulière |
...
| Tests baseline/repository/API | Tests ciblés du moteur, du repository et de la route                                                                                             | Réussis le 30 juillet 2026                                                             |
| Suite complète                | Vitest, 95 fichiers et 387 tests                                                                                                                 | Réussi le 30 juillet 2026                                                              |
| Qualité/build                 | TypeScript et `npm run build:dev`                                                                                                                | Réussi le 30 juillet 2026                                                              |
| Fondation SQL                 | `supabase/migrations/20260730105842_outcome_graph_foundation.sql` — 16 tables, contrôles append-only, workflow modèle, RLS sans grant navigateur | Présent; inclus dans le replay local réussi                                            |
| Ingestion SQL                 | `supabase/migrations/20260730141957_outcome_source_ingestion.sql` — 7 tables, bucket privé, jobs, provenance, matching, purge et checkpoints CAS | Présent; inclus dans le replay local réussi                                            |
| Pont catalogue SQL            | `supabase/migrations/20260730163756_outcome_catalogue_bridge.sql` — annonce durable, résultat inconnu/non entraînable et garde de suppression    | Présent; inclus dans le replay local réussi; backfill distant non exécuté              |
| Tests base/RLS                | Suites `supabase/tests/`                                                                                                                         | 274/274 assertions pgTAP réussies après replay des 84 migrations locales               |
| Connecteurs et pipeline       | `services/data-pipeline/src/official_sources/` et `src/outcome_ingestion/`                                                                       | 609 tests Python réussis hors `test_valuation_training.py`; aucun appel live Judilibre |
| CLI source                    | `python -m src.outcome_sources_cli plan                                                                                                          | validate-local                                                                         | ingest-local | match-dvf | judilibre-\*` | Plan et validations locales réussis; import distant non exécuté |
...
## Vérifications à exécuter avant fusion
...
Vérifié dans ce worktree le 30 juillet 2026 :
...
`test_valuation_training.py` n’est pas collectable sous l’interpréteur local Python 3.14; le projet cible Python 3.12. Ce fichier exclu doit donc être rejoué dans l’environnement Python cible avant fusion ou déploiement. Cette limite ne transforme pas les 609 autres succès en validation statistique du modèle.
...
1. Déployer ensemble le code du pipeline et les migrations sur staging, y rejouer pgTAP, exécuter le pont/backfill catalogue, puis répéter cette séquence contrôlée en production.
...
3. Alimenter le registre sans relier son cycle de vie destructivement au nettoyage de `auction_sales`; le pont fail-closed doit être complet avant toute suppression.
